import { parseWhatsAppChat, type ParsedMessage } from "./chat-parser";
import { lexicons, type LexiconCategory } from "./lexicons";

/** Messaggio arricchito con indice interno per segmentazione */
type IndexedMessage = ParsedMessage & { _i: number };

/** Mappe keyed by participant id (nome o alias anonimizzato) */
type ByParticipantNum = Record<string, number>;
type ByParticipantNumArr = Record<string, number[]>;
type ByParticipantSet = Record<string, Set<string>>;
type ByParticipantNullableNum = Record<string, number | null>;
type LongMessageCounts = Record<string, Record<string, number>>;

type LexLemmaAgg = { total: number; by_lemma: Record<string, number> };
type ParticipantLexiconAgg = Record<LexiconCategory, LexLemmaAgg>;
...
function inclusiveCalendarDaysBetween(firstDate: Date, lastDate: Date): number {
...
function daypartLabel(d: Date): "mattina" | "pomeriggio" | "sera" | "notte" {
  const h = d.getHours();
...
function segmentMessages(messages: IndexedMessage[], gapMinutes: number): IndexedMessage[][] {
  const gapMs = gapMinutes * 60 * 1000;
  const segments: IndexedMessage[][] = [];
  let cur: IndexedMessage[] = [];
...
function countLexiconHits(
  body: string,
  { lemmas = [], phrases = [] }: { lemmas?: readonly string[]; phrases?: readonly string[] }
) {
...
function messageHasQuestion(body: string): boolean {
  return body.includes("?");
}

function multiQuestionCount(body: string): number {
  return (body.match(/\?/g) || []).length;
}

function strongPunctuationHit(body: string): boolean {
...
function buildMessageIndexToSegment(segments: IndexedMessage[][]): Map<number, number> {
...
function questionAnsweredInWindow(
  messages: IndexedMessage[],
  segMap: Map<number, number>,
  _segments: IndexedMessage[][],
  qIndex: number,
  ...
  if (segIdx === undefined) return false;
...
function computeRelationalMetrics(
  messagesInput: ParsedMessage[],
  options: Partial<typeof DEFAULT_OPTIONS> = {}
) {
  const opt: typeof DEFAULT_OPTIONS = { ...DEFAULT_OPTIONS, ...options };
  const messages = messagesInput as IndexedMessage[];
  ...
  const daysByParticipant: ByParticipantSet = {};
  ...
  const messages_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const message_share_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
    message_share_by_participant[p] =
      total_messages > 0 ? round4(messages_by_participant[p] / total_messages) : 0;
  });

  const days_with_messages_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const conversation_starts_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
    conversation_starts_by_participant[p] = 0;
  });
  const segment_summaries: Array<{
    start_ts: number;
    end_ts: number;
    message_count: number;
    starter: string;
    messages_by_participant: ByParticipantNum;
  }> = [];

  for (const seg of segments) {
    const starter = seg[0].sender;
    conversation_starts_by_participant[starter] += 1;
    const byP: ByParticipantNum = {};
    participants.forEach((p) => {
      byP[p] = 0;
    });
    seg.forEach((m) => {
  ...
  const conversation_start_share: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const reopenings_after_long_pause_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const responseDelaysByParticipant: ByParticipantNumArr = {};
  const responseCountByParticipant: ByParticipantNum = {};
  const sameCalendarDayReplyFlags: { responder: string; sameDay: boolean }[] = [];
  participants.forEach((p) => {
  ...
  const median_response_time_by_participant: ByParticipantNullableNum = {};
  const p75_response_time_by_participant: ByParticipantNullableNum = {};
  participants.forEach((p) => {
  ...
  const same_day_reply_rate_by_participant: ByParticipantNullableNum = {};
  participants.forEach((p) => {
  ...
  const unanswered_segment_end_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const questions_by_participant: ByParticipantNum = {};
  const multi_question_messages_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
    questions_by_participant[p] = 0;
    multi_question_messages_by_participant[p] = 0;
  });

  let questionMessageIndices: number[] = [];
  ...
  const question_rate_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const avg_message_length_chars_by_participant: ByParticipantNum = {};
  const median_message_length_chars_by_participant: ByParticipantNum = {};
  const long_message_count_by_participant: LongMessageCounts = {};
  const short_message_rate_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const lengthsByP: ByParticipantNumArr = {};
  participants.forEach((p) => {
    lengthsByP[p] = [];
  });
  for (const m of messages) {
  ...
  const messages_per_day: Array<{ date: string; total: number; by_participant: ByParticipantNum }> = [];
  const dayMap = new Map<
    string,
    { date: string; total: number; by_participant: ByParticipantNum }
  >();
  for (const m of messages) {
    const dk = calendarDayKey(m.date);
    if (!dayMap.has(dk)) {
  ...
  [...dayMap.keys()]
    .sort()
    .forEach((k) => messages_per_day.push(dayMap.get(k)!));

  const weekMap = new Map<
    string,
    { week_start: string; total: number; by_participant: ByParticipantNum }
  >();
  for (const m of messages) {
  ...
  const monthMap = new Map<
    string,
    {
      month: string;
      total: number;
      by_participant: ByParticipantNum;
  ...
  const messages_per_month: Array<{ month: string; total: number; by_participant: ByParticipantNum }> =
    [];
  const active_days_per_month: Record<string, number> = {};
  const monthly_balance_by_participant: Record<string, ByParticipantNum> = {};
  ...
  const messagesByMonthIndex: Record<string, IndexedMessage[]> = {};
  ...
  const runLengthsByParticipant: ByParticipantNumArr = {};
  participants.forEach((p) => {
  ...
  const avg_consecutive_messages_by_participant: ByParticipantNum = {};
  const monologue_runs_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  for (const seg of segments) {
    ...
    const ok =
      ...
      participants.every((p) => byPBal[p] >= minEach);
    if (ok) {
      balanced_segments += 1;
    }
  }
  ...
  const messages_by_daypart: Record<
    DaypartKey,
    { total: number; by_participant: ByParticipantNum }
  > = {
  ...
  participants.forEach((p) => {
    (Object.keys(messages_by_daypart) as DaypartKey[]).forEach((k) => {
  ...
  const conversation_starts_by_daypart: Record<DaypartKey, ByParticipantNum> = {
  ...
  function lexiconAggForMessages(msgList: IndexedMessage[]) {
    const out: Record<string, ParticipantLexiconAgg> = {};
    participants.forEach((p) => {
      out[p] = {
    ...
    for (const m of msgList) {
      for (const kind of Object.keys(lexicons) as LexiconCategory[]) {
  ...
  function flattenLexTotals(agg: Record<string, ParticipantLexiconAgg>) {
    const row: Record<
      string,
      {
        affection_term_count: number;
        repair_signal_count: number;
        tension_marker_count: number;
        playfulness_marker_count: number;
        checkin_marker_count: number;
        care_marker_count: number;
      }
    > = {};
    participants.forEach((p) => {
      row[p] = {
  ...
  const strong_punctuation_count_by_participant: ByParticipantNum = {};
  participants.forEach((p) => {
  ...
  const monthly_question_rate: Record<string, number> = {};
  const monthly_affection_signal_rate: Record<string, number> = {};
  const monthly_lexical: Record<
    string,
    {
      affection_term_count_by_participant: ByParticipantNum;
      repair_signal_count_by_participant: ByParticipantNum;
      tension_marker_count_by_participant: ByParticipantNum;
      playfulness_marker_count_by_participant: ByParticipantNum;
    }
  > = {};
  ...
  for (const mk of Object.keys(messagesByMonthIndex).sort()) {
    ...
    monthly_lexical[mk] = {
      affection_term_count_by_participant: {} as ByParticipantNum,
      repair_signal_count_by_participant: {} as ByParticipantNum,
      tension_marker_count_by_participant: {} as ByParticipantNum,
      playfulness_marker_count_by_participant: {} as ByParticipantNum
    };
    participants.forEach((p) => {
  ...
  const participant_metrics: Record<string, Record<string, unknown>> = {};
  participants.forEach((p) => {
...
function emptyReport(opt: Partial<typeof DEFAULT_OPTIONS>, participants: string[]) {
  return {
    schema_version: 1,
...
function emptyFromError(message: string) {
  return {
    schema_version: 1,
    error: message,
    chat_metadata: {},
...
export function buildRelationalMetricsReport(
  chatText: string,
  options: Partial<typeof DEFAULT_OPTIONS> = {}
) {
  try {
    ...
    return emptyFromError(msg);
  }
}

export { computeRelationalMetrics, parseWhatsAppChat, DEFAULT_OPTIONS };
